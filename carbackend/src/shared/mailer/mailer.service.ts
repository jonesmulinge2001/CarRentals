...
export class MailerService {
    ...
    async sendWelcomeEmail(to: string, context: WelcomeEmailContext): Promise<void>{
        const emailOptions: EmailOptions = {
            to,
            subject: `Welcome to our platform Car Rentals`,
            template: 'email/welcome',
            context: {
                ...context,
                name: context.username,
                storeName: context.rentalName,
                loginUrl: context.loginUrl ||  `${this.configService.get<string>('FRONTEND_URL', 'http://localhost:3000/auth/login')}`,
                supportEmail: context.supportEmail || this.configService.get<string>('SUPPORT_EMAIL','jonathanmulingemwandi@gmail.com'),
                shoppieName: context.rentalName || 'Car Rentals',
